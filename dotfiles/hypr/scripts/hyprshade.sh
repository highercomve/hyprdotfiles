#!/bin/bash
# shellcheck source=../user_settings/hyprshade.sh
#  _   _                      _               _
...
if [[ "$1" == "rofi" ]]; then
    ...
    if [ ! -z "$choice" ]; then
        echo "hyprshade_filter=\"$choice\"" >"$HOME"/.config/hypr/user_settings/hyprshade.sh
        if [ "$choice" == "off" ]; then
            hyprshade off
    ...
    # shellcheck source=../user_settings/hyprshade.sh
    if [ -f "$HOME"/.config/hypr/user_settings/hyprshade.sh ]; then
        source "$HOME"/.config/hypr/user_settings/hyprshade.sh
    fi
